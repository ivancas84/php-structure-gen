...
class GetEntityTreeElement extends GenerateEntity {
  protected $names = [];
...
";
  ...
  protected function fk($entity, array $tablesVisited, $tab, $prefix = "") {
    ...
    foreach ($fk as $field) {
      ...
      $this->string .= "
" .$tab . "'{$prefixTemp}' => ['field_id'=>'{$fieldId}', 'field_name'=>'{$field->getName()}', 'entity_name'=>'{$field->getEntityRef()->getName()}', 'children'=>[";
      
      if(!in_array($field->getEntityRef()->getName(), $tablesVisited)) $this->fk($field->getEntityRef(), $tablesVisited, $tab . "  ", $prefixTemp);
